					...
					{[
						{
							...
							description: 'Designed and implemented a microservices architecture to Query the LLM that designed from scratch',
							//handling 1M+ daily requests with 99.99% uptime',
							details: [
								'Developed an LLM encoder in Scala and trained the model on the cloud.',
								'Generated Byte Pair Codings (BPE’s) for training data set using Jtokkit, map-reduce and deployed the pipeline successfully using AWS-EMR for distributed processing',
								'Trained the decoder neural network with Apache Spark, implemented an attention mechanism, and validated overall model performance.',
								'Deployed the model as a step function on AWS Lambda and integrated it with AWS API Gateway',
							],
							tech: ['Scala', 'Deeplearning4j', 'Docker', 'Map-Reduce', 'Spark', 'AWS(EMR, S3, Lambda)'],
						},
						{
